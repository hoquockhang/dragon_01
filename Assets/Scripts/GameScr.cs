...
public class GameScr : mScreen, IChatable
{
    ...
    public void doDoubleClickToObj1(IMapObject obj)
    {
        //if (BoardEvent.isShow) return;
    ...
    // Hàm tạo hiệu ứng chữ bay lên (Fly Text)
    public static void startFlyText(string flyString, int x, int y, int dx, int dy, int color)
    {
        int num = -1;

        // Tìm slot trống trong danh sách fly text (chỉ cho tối đa 5 text bay cùng lúc)
        for (int i = 0; i < 5; i++)
        {
            if (flyTextState[i] == -1) // nếu state = -1 => slot rỗng
            {
                num = i;
                break;
            }
        ...
        if (num == -1) // Nếu không còn chỗ trống thì thoát (không tạo thêm fly text)
        {
            return;
        }

        // Lưu thông tin fly text vào slot được chọn
        flyTextColor[num] = color;      // màu của chữ
        flyTextString[num] = flyString; // nội dung text muốn hiển thị
        flyTextX[num] = x;              // vị trí X
        flyTextY[num] = y;              // vị trí Y
        flyTextDx[num] = dx;            // tốc độ di chuyển ngang

        // Nếu dy >= 0 thì bay lên, ngược lại thì bay xuống
        flyTextDy[num] = ((dy >= 0) ? 5 : (-5));

        flyTextState[num] = 0;  // trạng thái bắt đầu (0 = mới tạo)
        flyTime[num] = 0;       // thời gian hiển thị text
        flyTextYTo[num] = 10;   // khoảng cách Y mục tiêu (để text bay đến vị trí này)

        // Xử lý tránh chồng chữ lên nhau nếu có nhiều fly text trùng vị trí
        for (int j = 0; j < 5; j++)
        {
            // Nếu slot khác đang có fly text + khoảng cách X gần nhau + bay lên + cùng vị trí YTo
            if (flyTextState[j] != -1 && num != j && flyTextDy[num] < 0 && Res.abs(flyTextX[num] - flyTextX[j]) <= 20 && flyTextYTo[num] == flyTextYTo[j])
            {
                flyTextYTo[num] += 10; // Đẩy text mới xuống 1 chút cho không đè lên text cũ
            }
        }
    }
