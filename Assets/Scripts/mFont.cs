...
public class mFont
{
    ...
    public static mFont tahoma_7b_cyan;
    ...
    public static mFont BIG_DAME;   // 🔥 Font dame to gấp 3
    ...
    public int color = -1;
    public mFont(int style, int size, bool bold, int color)
    {
        try
        {
            this.space = 1; // khoảng cách ký tự mặc định
            this.color = color;

            // Xác định font gốc theo size
            string fontName = "tahoma_" + size + (bold ? "b" : "");
            string pathImage = "/myfont/" + fontName + ".png";
            string pathData = "/myfont/" + fontName;

            // Load dữ liệu font như constructor gốc
            this.strFont = " 0123456789+-*='_?.,<>/[]{}!@#$%^&*():aáàảãạâấầẩẫậăắằẳẵặbcdđeéèẻẽẹêếềểễệfghiíìỉĩịjklmnoóòỏõọôốồổỗộơớờởỡợpqrstuúùủũụưứừửữựvxyýỳỷỹỵzwAÁÀẢÃẠĂẰẮẲẴẶÂẤẦẨẪẬBCDĐEÉÈẺẼẸÊẾỀỂỄỆFGHIÍÌỈĨỊJKLMNOÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢPQRSTUÚÙỦŨỤƯỨỪỬỮỰVXYÝỲỶỸỴZW";

            this.pathImage = pathImage;
            DataInputStream dis = null;
            reloadImage(); // nạp ảnh font

            try
            {
                dis = MyStream.readFile(pathData);
                fImages = new int[dis.readShort()][];

                for (int i = 0; i < fImages.Length; i++)
                {
                    fImages[i] = new int[4];
                    fImages[i][0] = dis.readShort();
                    fImages[i][1] = dis.readShort();
                    fImages[i][2] = dis.readShort();
                    fImages[i][3] = dis.readShort();
                    setHeight(fImages[i][3]);
                }
                dis.close();
            }
            ...
            {
                try { if (dis != null) dis.close(); } catch { }
            }
        }
        catch (Exception ex)
        {
    ...
    public static void init()
	{
		...
		{
            tahoma_7b_cyan = new mFont(0, 7, false, 0x00FFFF);
